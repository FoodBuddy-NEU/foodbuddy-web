...
test("logs in with email/password and redirects to home", async () => {
  ...
  expect(signInWithEmailAndPassword).toHaveBeenCalledWith(expect.any(Object), "user@example.com", "secret");
  await waitFor(() => expect(mockPush).toHaveBeenCalledWith("/"));
});

test("shows error message on email/password login failure", async () => {
  mockEmailLogin.mockRejectedValue(new FirebaseError("auth/invalid-credential", "Invalid credentials"));
